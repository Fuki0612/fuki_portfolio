...
const HistoryCard: React.FC<{
  ...
  return (
    ...
    {index % 2 === 0 ? (
      ...
      >
        {index === 0 ? (
          <div className="absolute top-1/2 left-1/2 bg-yellow-300 w-36 h-1" />
        ) : isLast ? (
          <div className="absolute top-1/2 right-1/2 bg-yellow-300 w-36 h-1" />
        ) : (
          <div className="absolute top-1/2 bg-yellow-300 w-72 h-1" />
        )}
      ...
      <div className="relative flex flex-col items-center"
        ...
        onMouseLeave={() => !alwaysShowDetail && setIsHovering(false)}
      >
        <div className="flex flex-col-reverse items-center w-72 h-32">
          <div className={`${ShipporiMinchoFont.className} text-white text-xl md:text-2xl font-bold`}>
            {year}
          </div>
          <div className={`${ShipporiMinchoFont.className} text-white text-lg md:text-xl font-bold text-center`}>
